 ...
 * @param __Tx_Data_Type 传输数据类型, 默认float
 * @param __Frame_Header 帧头标识符
 */
void Class_Serialplot::Init(UART_HandleTypeDef *huart, Enum_Serialplot_Checksum_8 __Checksum_8, uint8_t __Rx_Variable_Assignment_Num, char **__Rx_Variable_Assignment_List, Enum_Serialplot_Data_Type __Tx_Data_Type, uint8_t __Frame_Header)
{
...
void Class_Serialplot::TIM_Write_PeriodElapsedCallback()
{
    ...
    HAL_UART_Transmit_DMA(UART_Manage_Object->UART_Handler, UART_Manage_Object->Tx_Buffer, data_length);
}
...
void Class_Serialplot::_Judge_Variable_Value(int flag)
{
    ...
    // 计算值并注意小数点是否存在及其位置
    for (i = flag; UART_Manage_Object->Rx_Buffer[i] != '#' && UART_Manage_Object->Rx_Buffer[flag] != 0; i++)
    {
...
void Class_Serialplot::Output()
{
    ...
    else if (Tx_Data_Type == Serialplot_Data_Type_UINT32 || Tx_Data_Type == Serialplot_Data_Type_INT32 || Tx_Data_Type == Serialplot_Data_Type_FLOAT)
    {
        ...
        if(Checksum_8 != Serialplot_Checksum_8_DISABLE)
        {
            tmp_buffer[1 + Data_Number * sizeof(uint32_t)] = Math_Sum_8(tmp_buffer + 1, 1 + Data_Number * sizeof(uint32_t));
        }
    }
